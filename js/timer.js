...
var end = new Date(new Date().getUTCFullYear(), 8, 15, 0, 0);
...
function showRemaining() {
  ...
  if ( can ) {
    ...
    context.fillStyle = "rgb(51, 51, 50)";
...
function splitDigit( position, unit ) {
  ...
  if( position==-1&&unit.length<3) unit="0" + unit;
  var d = unit.split("");
  drawDigit(position, d[0]);
  drawDigit(position+1, d[1]);
  if(unit.length==3)
    drawDigit(position+2, d[2]);
}
...
function drawDigit(position, num) {
  ...
  for ( var i = 0; i < chr.length; i++ ) {
    ...
    drawBlip(
      ...
      25 + y * 12,
      (y % 2 == 0),
      c == 1 ? "rgba(255,255,255,1)" : "rgba(255,255,255,0.1)"
    );
  }
}
